# app/presenters/sidebar_presenter.rb
class SidebarPresenter
  include Rails.application.routes.url_helpers
  include ActionView::Helpers::TagHelper
  include ActionView::Helpers::UrlHelper
  include ActionView::Context

  attr_reader :view_context

  def initialize(view_context, collapsed: false)
    @view_context = view_context
    @collapsed    = collapsed
  end

  def render_items
    grouped = authorized_items.group_by { |item| item[:group] }
    group_order = [nil, 'Ventas', 'Operaciones', 'Inventario', 'Finanzas', 'Configuración']

    group_order.filter_map do |group_name|
      items = grouped[group_name]
      next unless items&.any?
      render_group(group_name, items)
    end.join.html_safe
  end

  private

  attr_reader :collapsed

  # =========================
  #   AUTORIZACIÓN / ROLES
  # =========================
  def current_user
    view_context.current_user
  end

  def authorized_items
    items.select { |it| visible_for_roles?(it) && passes_condition?(it) }
  end

  def visible_for_roles?(item)
    roles = Array(item[:roles]) # nil -> []
    return true if roles.empty? # sin restricción explícita

    # admite símbolos o strings
    roles.any? { |r| current_user.has_role?(r.to_sym) }
  end

  # Útil si querés Pundit/CanCan/predicados personalizados:
  # { if: ->(vc, user){ vc.policy(User).index? } }
  # { if: ->(vc, user){ vc.can?(:read, User) } }
  def passes_condition?(item)
    cond = item[:if]
    return true unless cond.respond_to?(:call)
    !!cond.call(view_context, current_user)
  end

  # =========================
  #         ÍTEMS
  # =========================
  def items
    [
      # ── Sin grupo (siempre arriba) ────────────────────────────────────────────
      {
        name: 'Inicio',
        path: root_path,
        roles: %i[superadmin vendedor cajero],
        icon: 'M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25'
      },

      # ── Ventas ───────────────────────────────────────────────────────────────
      {
        name: 'Órdenes',
        group: 'Ventas',
        path: orders_path,
        roles: %i[superadmin vendedor cajero mesero],
        icon: 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-1.125 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z'
      },
      {
        name: 'Fiado',
        group: 'Ventas',
        path: pending_orders_path,
        roles: %i[superadmin cajero],
        icon: 'M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z'
      },
      {
        name: 'Delivery',
        group: 'Ventas',
        path: delivery_index_path,
        roles: %i[delivery superadmin],
        icon: 'M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12'
      },
      {
        name: 'Mesas',
        group: 'Ventas',
        path: tables_path,
        roles: %i[superadmin vendedor cajero mesero],
        icon: 'M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z'
      },

      # ── Operaciones ──────────────────────────────────────────────────────────
      {
        name: 'Clientes',
        group: 'Operaciones',
        path: customers_path,
        roles: %i[superadmin vendedor cajero],
        icon: 'M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z'
      },
      {
        name: 'Caja',
        group: 'Operaciones',
        path: cash_registers_path,
        roles: %i[superadmin cajero],
        icon: 'M4 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v1H4V4zm0 3h16v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7zm6 2v2m4-2v2M6 9h.01M6 12h.01M6 15h.01M6 18h.01'
      },

      # ── Inventario ───────────────────────────────────────────────────────────
      {
        name: 'Productos',
        group: 'Inventario',
        path: hub_products_path,
        roles: %i[superadmin],
        icon: 'M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z'
      },
      {
        name: 'Categorías',
        group: 'Inventario',
        path: categories_path,
        roles: %i[superadmin],
        icon: 'M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z'
      },
      {
        name: 'Stock',
        group: 'Inventario',
        path: stocks_path,
        roles: %i[superadmin cajero],
        icon: 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125'
      },
      {
        name: 'Compras',
        group: 'Inventario',
        path: purchases_path,
        roles: %i[superadmin],
        icon: 'M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z'
      },
      {
        name: 'Proveedores',
        group: 'Inventario',
        path: suppliers_path,
        roles: %i[superadmin],
        icon: 'M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12'
      },

      # ── Finanzas ─────────────────────────────────────────────────────────────
      {
        name: 'Gastos',
        group: 'Finanzas',
        path: expenses_path,
        roles: %i[superadmin],
        icon: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z'
      },
      {
        name: 'Reportes',
        group: 'Finanzas',
        path: reports_path,
        roles: %i[superadmin],
        icon: 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z'
      },

      # ── Configuración ────────────────────────────────────────────────────────
      {
        name: 'Usuarios',
        group: 'Configuración',
        path: users_path,
        roles: %i[superadmin],
        icon: 'M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z'
      },
      {
        name: 'Métodos de Pago',
        group: 'Configuración',
        path: payment_methods_path,
        roles: %i[superadmin cajero],
        icon: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z'
      },
      {
        name: 'Monedas',
        group: 'Configuración',
        path: currencies_path,
        roles: %i[superadmin],
        icon: 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z'
      },
      {
        name: 'Unidades',
        group: 'Configuración',
        path: units_path,
        roles: %i[superadmin],
        icon: 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5'
      }
    ]
  end

  def render_group(name, group_items)
    items_html = group_items.map { |item| render_item(item) }.join.html_safe

    if name.nil? || collapsed
      items_html
    else
      label = content_tag(:p, name, class: 'px-3 pt-1 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-widest')
      content_tag(:li) do
        label + content_tag(:ul, items_html, role: 'list', class: '-mx-2 space-y-1')
      end
    end
  end

  def render_item(item)
    is_current = view_context.current_page?(item[:path])

    if collapsed
      render_collapsed_item(item, is_current)
    else
      render_expanded_item(item, is_current)
    end
  end

  def render_collapsed_item(item, is_current)
    link_class = is_current ?
      'group flex justify-start items-center rounded-xl bg-indigo-50 p-4 pl-3 text-sm/6 font-semibold text-indigo-700 border border-indigo-200' :
      'group flex justify-start items-center rounded-xl p-4 pl-3 text-sm/6 font-semibold text-gray-600 hover:bg-gray-50 hover:text-indigo-600 transition-colors duration-100'

    icon_class = is_current ?
      'size-6 shrink-0 text-indigo-600' :
      'size-6 shrink-0 text-gray-400 group-hover:text-indigo-600 transition-colors duration-200'

    content_tag(:li) do
      link_to(item[:path], class: link_class, title: item[:name], data: { tooltip: item[:name] }) do
        content_tag(:svg, class: icon_class, fill: 'none', viewBox: '0 0 24 24', "stroke-width": '2', stroke: 'currentColor') do
          content_tag(:path, nil, "stroke-linecap": 'round', "stroke-linejoin": 'round', d: item[:icon])
        end
      end
    end
  end

  def render_expanded_item(item, is_current)
    link_class = is_current ?
      'group flex gap-x-3 rounded-xl bg-indigo-50 p-3 text-sm/6 font-semibold text-indigo-700 border border-indigo-200' :
      'group flex gap-x-3 rounded-xl p-3 text-sm/6 font-semibold text-gray-600 hover:bg-gray-50 hover:text-indigo-600 transition-colors duration-100'

    icon_class = is_current ?
      'size-6 shrink-0 text-indigo-600' :
      'size-6 shrink-0 text-gray-400 group-hover:text-indigo-600 transition-colors duration-200'

    content_tag(:li) do
      link_to(item[:path], class: link_class) do
        svg_tag = content_tag(:svg, class: icon_class, fill: 'none', viewBox: '0 0 24 24', "stroke-width": '2', stroke: 'currentColor') do
          content_tag(:path, nil, "stroke-linecap": 'round', "stroke-linejoin": 'round', d: item[:icon])
        end

        svg_tag + content_tag(:span, item[:name], data: { "sidebar-collapse-target": 'content' }, class: 'transition-all duration-200')
      end
    end
  end
end
